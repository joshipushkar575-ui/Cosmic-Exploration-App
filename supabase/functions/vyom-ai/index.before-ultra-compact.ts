...
LIVE ASTRONOMY DATA:
${JSON.stringify({
  location: compactAstronomy.location,
  sun: compactAstronomy.sun,
  moon: compactAstronomy.moon,
  planets: Array.isArray(compactAstronomy.planets)
    ? compactAstronomy.planets.slice(0, 8)
    : [],
  ar00: compactAstronomy.ar00,
  nasa: compactAstronomy.nasa
    ? {
        apodTitle: compactAstronomy.nasa.apodTitle,
        spaceWeather: compactAstronomy.nasa.spaceWeather
      }
    : null
})}

Answer the user's question accurately and concisely.
Use live data only when relevant.
Never invent unavailable astronomical values.
`;
