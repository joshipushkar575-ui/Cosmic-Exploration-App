...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const userQuestion = String(message).slice(0, 4000);
...
const wantsNASA =
  q.includes("nasa") ||
  q.includes("apod") ||
  q.includes("space news") ||
  q.includes("space news") ||
  q.includes("latest discovery") ||
  q.includes("latest discovery") ||
  q.includes("space mission") ||
  q.includes("space missions") ||
  q.includes("james webb") ||
  q.includes("jwst") ||
  q.includes("artemis") ||
  q.includes("iss") ||
  q.includes("international space station") ||
...
if (wantsMoon) relevantData.moon = compactAstronomy.moon;
if (wantsSun) relevantData.sun = compactAstronomy.sun;
if (wantsPlanet) {
  relevantData.planets = Array.isArray(compactAstronomy.planets)
    ? compactAstronomy.planets.slice(0, 8)
    : [];
}

if (wantsSky) {
  relevantData.location = compactAstronomy.location;
  relevantData.moon = compactAstronomy.moon;
  relevantData.sun = compactAstronomy.sun;
  relevantData.planets = Array.isArray(compactAstronomy.planets)
    ? compactAstronomy.planets.slice(0, 8)
    : [];
  relevantData.ar00 = compactAstronomy.ar00;
}
...
if (wantsNASA) {
  relevantData.nasa = {
    available: true,
    instruction:
      "Use general established NASA/space knowledge. Do not invent current mission details. If the user asks for latest/current NASA information, clearly state that live NASA data is not available in this AI request."
  };
}

const isroContextInstruction = isroContext
  ? `
VYOM ISRO VERIFIED CONTEXT:

${JSON.stringify(isroContext)}

STRICT ISRO RULES:
- Treat the supplied ISRO context as verified mission information for this response.
- Use these supplied facts when answering mission-specific questions.
- Do NOT invent mission status, instruments, measurements, discoveries, dates, or numerical values.
...
RELEVANT VYOM LIVE DATA:
${JSON.stringify(relevantData)}

Use the supplied live data when relevant.
Never invent missing astronomical values.
For general science questions, answer from established scientific knowledge.
