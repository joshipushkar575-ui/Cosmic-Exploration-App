      ...
      if (isJplPlanetQuestion && jplPlanetMatch) {
        ...
        try {
          ...
          if (jplResult.success && jplResult.ephemeris) {
            ...
            const nextRiseText =
              typeof localPlanet?.visibility === 'string' &&
              /rises at/i.test(localPlanet.visibility)
                ? localPlanet.visibility.replace(/^.*?(?=Rises at)/i, '')
                : null;
            ...
            const validationText =
              altitudeDifference !== null
                ? Math.abs(altitudeDifference) <= 2
                  ? '✓ JPL and VYOM local altitude calculations are closely aligned.'
                  : Math.abs(altitudeDifference) <= 5
                    ? 'ℹ️ JPL and VYOM local altitude calculations differ slightly.'
                    : '⚠️ JPL and VYOM local altitude calculations show a larger difference.'
                : null;
            ...
            const jplResponse =
              ...
              (visibilityText
                ...
                : '') +
              (nextRiseText && !jplAboveHorizon
                ? `• Next rise: **${nextRiseText}**\n`
                : '') +
              (constellationName
