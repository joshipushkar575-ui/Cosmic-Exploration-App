    ...
    return (
      <div className="space-y-6">
        <div className="text-center">
          <h3 className="text-2xl font-bold text-white mb-2">Advanced Astronomy Quiz</h3>
          <p className="text-gray-300">Question {currentQuestion + 1} of {quizQuestions.length}</p>
          <Progress value={(currentQuestion / quizQuestions.length) * 100} className="mt-2" />
        </div>

        <GlassCard className="p-6">
          <h4 className="text-xl text-white mb-4">{quizQuestions[currentQuestion].question}</h4>
  ...
  return (
    <CosmicBackground variant="galaxy">
      <div className="min-h-screen p-4">
        ...
        >
          <h1 className="text-4xl font-bold text-white mb-2">Space Academy</h1>
          <p className="text-gray-300">Challenge yourself. Compete globally. Reach for the stars.</p>
        ...
        <Tabs defaultValue="competitions" className="max-w-6xl mx-auto">
          ...
          <TabsContent value="competitions">
            <div className="space-y-4">
              <h2 className="text-2xl font-bold text-white mb-4">Global Competitions</h2>
                ...
                >
                  <GlassCard className="p-6">
                    <div className="flex items-start gap-4">
                      <div className={`p-3 ${comp.color} rounded-lg flex-shrink-0`}>
                      ...
                      <div className="flex-grow">
                        <div className="flex items-start justify-between mb-2">
                          ...
                          <Badge 
                            ...
                            className={comp.status === 'active' ? 'bg-green-600' : ''}
                          >
                            {comp.status}
                          </Badge>
                        </div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                          <div>
                            ...
                            <p className="text-white font-medium">{comp.difficulty}</p>
                          </div>
                          <div>
                            <span className="text-gray-400">Participants:</span>
                            <p className="text-white font-medium">{comp.participants.toLocaleString()}</p>
                          </div>
                          <div>
                            <span className="text-gray-400">Time Left:</span>
                            <p className="text-white font-medium">{comp.timeLeft}</p>
                          </div>
                          <div>
                            <span className="text-gray-400">Prize:</span>
                            <p className="text-yellow-400 font-medium">{comp.prize}</p>
                          </div>
                        </div>
                        <Button className="mt-4" disabled={comp.status !== 'active'}>
          ...
          <TabsContent value="challenges">
            <div className="space-y-4">
              <h2 className="text-2xl font-bold text-white mb-4">Daily & Weekly Challenges</h2>
                  ...
                  <GlassCard className={`p-4 ${challenge.completed ? 'bg-green-500/20' : ''}`}>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        <challenge.icon className={`w-8 h-8 ${challenge.completed ? 'text-green-400' : 'text-white'}`} />
                        <div>
                          <h3 className="text-white font-bold">{challenge.title}</h3>
                          <p className="text-gray-300 text-sm">{challenge.description}</p>
                          <div className="flex items-center gap-4 mt-2">
                            <Badge variant="outline">{challenge.difficulty}</Badge>
                            <span className="text-yellow-400 text-sm">+{challenge.points} pts</span>
                  ...
                  {leaderboard.map((user, index) => (
                    <motion.div
                      ...
                      className={`flex items-center justify-between p-3 rounded-lg ${
                        user.name === 'You' ? 'bg-yellow-500/20' : 'bg-white/5'
                      }`}
                    >
                      <div className="flex items-center gap-4">
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
                          user.rank <= 3 ? 'bg-gradient-to-r from-yellow-400 to-orange-500' : 'bg-gray-600'
                        }`}>
                          <span className="text-white font-bold">#{user.rank}</span>
                        </div>
                        <div>
                          <h3 className={`font-bold ${user.name === 'You' ? 'text-yellow-400' : 'text-white'}`}>
                            {user.name}
                          </h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {achievements.map((achievement, index) => (
                  <motion.div
                    ...
                    transition={{ delay: 0.1 * index }}
                  >
                    <GlassCard className={`p-6 ${achievement.earned ? 'bg-gradient-to-br from-yellow-500/20 to-orange-500/20' : 'bg-gray-500/20'}`}>
                      <div className="text-center">
