      ...
      <div className="min-h-screen p-4">
        ...
        >
          <div className="flex items-center">
            ...
            <div>
              <h1 className="bg-gradient-to-r from-white via-cyan-100 to-violet-200 bg-clip-text text-2xl font-bold tracking-tight text-transparent">3D Cosmic Explorer</h1>
          ...
          <div className="flex gap-2">
            <Button
              ...
              className={`rounded-xl border px-3 py-2 text-sm transition-all duration-300 cursor-pointer ${
                viewMode === '3d'
                  ? 'border-cyan-300/25 bg-cyan-300/[0.10] text-white shadow-[0_0_24px_rgba(34,211,238,0.12)]'
                  : 'border-white/[0.10] bg-white/[0.035] text-slate-400 hover:border-white/[0.18] hover:bg-white/[0.07] hover:text-white'
              }`}
            >
              <Eye className="mr-1.5 inline h-4 w-4 text-cyan-300" />
              3D View
            </Button>
            <Button
              ...
              className={`rounded-xl border px-3 py-2 text-sm transition-all duration-300 cursor-pointer ${
                viewMode === 'ar'
                  ? 'border-violet-300/25 bg-violet-300/[0.10] text-white shadow-[0_0_24px_rgba(139,92,246,0.12)]'
                  : 'border-white/[0.10] bg-white/[0.035] text-slate-400 hover:border-white/[0.18] hover:bg-white/[0.07] hover:text-white'
              }`}
            >
              <Maximize className="mr-1.5 inline h-4 w-4 text-violet-300" />
              AR Mode
            </Button>
          </div>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 h-[calc(100vh-160px)]">
          ...
          >
            <GlassCard className="h-full p-5 flex flex-col rounded-3xl border-white/[0.14]" glow intensity="medium">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  <h2 className="text-lg font-semibold tracking-tight text-white">Solar System</h2>
                  <span className="rounded-full border border-cyan-300/15 bg-cyan-300/[0.06] px-2.5 py-1 text-[11px] font-medium text-cyan-200 shadow-[0_0_18px_rgba(34,211,238,0.06)]">
                ...
                <div className="flex gap-2">
                  <CosmicIconButton
                    ...
                    size="sm"
                    glow="cyan"
                    label={isAnimating ? 'Pause motion' : 'Play motion'}
                  >
              ...
              <div className="relative flex-1 overflow-hidden rounded-2xl border border-white/[0.10] bg-black/50 shadow-[inset_0_1px_0_rgba(255,255,255,0.05),0_12px_40px_rgba(0,0,0,0.25)]">
                {viewMode === 'ar' ? <SolarSystemAR planets={planets} /> : <OriginalSolarSystem3D />}

                {/* View Mode Indicator */}
                <div className="absolute top-4 right-4 z-50">
                  <GlassCard className="rounded-full border-white/[0.12] bg-slate-950/55 px-3 py-1.5 shadow-[0_8px_25px_rgba(0,0,0,0.25)]" intensity="light">
          ...
          >
            <GlassCard className="h-full p-5 flex flex-col overflow-y-auto rounded-3xl border-white/[0.14]" intensity="medium">
              <h2 className="mb-4 flex items-center gap-2 text-lg font-semibold tracking-tight text-white">
              ...
              {selectedPlanet ? (
                ...
                >
                  <div className="text-center">
                    <div 
                      className="mx-auto mb-3 h-20 w-20 rounded-full border border-white/15 ring-1 ring-white/10"
                      style={{ 
                        backgroundColor: selectedPlanet.color,
                        boxShadow: `0 0 30px ${selectedPlanet.color}60`
                      }}
                    />
                    <h3 className="text-xl font-bold tracking-tight text-white">{selectedPlanet.name}</h3>
                    <p className="text-xs text-violet-300 font-mono mt-0.5">Helio Lon: {selectedPlanet.currentPosition}°</p>
                  </div>

                  <div className="space-y-3">
                    <div className="grid grid-cols-2 gap-2 text-left">
                      <div className="rounded-xl border border-white/[0.08] bg-white/[0.045] p-3 transition-colors hover:bg-white/[0.07]">
                  ...
                  <Button
                    className="w-full rounded-xl border border-cyan-200/20 bg-gradient-to-r from-violet-500/90 to-cyan-400/85 text-white shadow-[0_8px_26px_rgba(34,211,238,0.12)] hover:from-violet-400 hover:to-cyan-300 cursor-pointer"
        ...
        <motion.div
          ...
          className="mt-6"
        >
          <GlassCard className="rounded-2xl border-white/[0.12] p-4" intensity="light">
                ...
                <div className="flex gap-1.5">
                  {[0.5, 1, 2].map((spd) => (
                    <Button
                      ...
                      className={`rounded-xl px-3 text-xs cursor-pointer transition-all duration-300 ${
                        speedMultiplier === spd
                          ? 'border border-cyan-300/25 bg-cyan-300/[0.10] text-cyan-100 shadow-[0_0_18px_rgba(34,211,238,0.10)]'
                          : 'border border-transparent bg-white/[0.025] text-slate-400 hover:border-white/[0.12] hover:bg-white/[0.06] hover:text-white'
                      }`}
                    >
                      {spd}x
                    </Button>
